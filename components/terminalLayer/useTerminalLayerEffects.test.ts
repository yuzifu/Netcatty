...
test("terminal activity guards skip parsing when activity is already known", () => {
  ...
  assert.ok(visibleGuardIndex < filterIndex);
  assert.ok(alreadyActiveGuardIndex < filterIndex);
});
